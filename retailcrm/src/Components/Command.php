...
class Command
{
    ...
    {
        $builder = new CallsBuilder();
        $calls = array();

        if ($this->uid) {
            $calls = $builder->buildCallsById($this->uid);
        } elseif ($this->limit) {
            $calls = $builder->buildCallsLast();
        } else {
            $calls = $builder->buildCalls();
        }

        if (!empty($calls)) {
            $this->requestHelper->uploadOrders($calls, true, 'calls');
        }  
    }
    
    /**
     * Subscribers as customers
     */
    public function runSubscribers()
    {
        $builder = new SubscribersBuilder();
        $subscribers = array();
        
        if ($this->uid) {
            $subscribers = $builder->buildSubscribersById($this->uid);
        } elseif ($this->limit) {
            $subscribers = $builder->buildSubscribersLast();
        } else {
            $subscribers = $builder->buildSubscribers();
        }

        if (!empty($subscribers)) {
            $this->requestHelper->uploadCustomers($subscribers);
        }
    }
