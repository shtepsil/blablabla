...
class ApiHelper
{
    ...
    public function uploadOrders($orders, $check = true, $logfile = '')
    {
        ...
        if (!empty($logfile)) {
            switch ($logfile) {
                case 'calls':
                    $this->logger->put($timemark, $this->container->callsLog);
                    break;
            }
        } else {
    ...
    public function updateCustomers($customers, $searchEdit = false)
    {
        ...
        $this->logger->put($timemark, $this->container->customersUpdatesLog);
    }
