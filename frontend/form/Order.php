...
//            $data['user_phone'] = $user->phone;
        ...
        // Текущий день недели (число)
        $current_day = DateHelper::getDayOfWeek('number');
        // Начало текущего дня (timestamp)
        $current_day_start = DateHelper::currentDayStart();

        // Timestamp начала текущего дня ПЛЮС 11 часов.
        $today_delivery = $current_day_start + 39600;

        // Если выходные
        if(DateHelper::weekendCheck()){
            // Если суббота
            if ($current_day == 6) {
                // В субботу добавим 48 часов (2 суток)
                $time += 172800;
            }
            // Если воскресенье
            if ($current_day == 7) {
                // В воскресенье добавим 24 часа (1 сутки)
                $time += 86400;
            }
        } elseif ($current_day == 5) {
            // Если пятница
            /*
             * Если пятница, и уже 11:00,
             * то перенесём доставку на понедельник, а не на субботу.
             * Плюс 3 суток.
             */
            ...
            // Если будние дни с понедельника по четверг.
            /*
        ...
        if ($sessions_items) {
            ...
            foreach ($items as $key => $item) {
                ...
                /*
                 * Тут закомментировал потому что в новом функционале
                 * оптовая цена на товар формируется и в методе $item->real_price тоже
                 */
...
//        d::ajax($data['full_price']);
        ...
        if(in_array($data['payment'], [2, 4])){
            $data['pay_status'] = 'wait';
        }
//        d::ajax($data['full_price'] . ' - form/Order.php 639');
        /**
         * Проверка на минимальную сумму заказа для выбранного города
         * $min_order_amount либо число либо NULL
         * $this->city - это ID города, выбранного в select'e городов.
         */
        $min_order_amount = City::getMinOrderAmount($this->city);
        if($min_order_amount AND $data['full_price'] < $min_order_amount){
            $message_error = 'Минимальная сумма заказа для выбранного города';

            if($this->type_delivery == 3){
                $message_error .= ', без учёта Яндекс доставки';
            }
            $message_error .= ' ' . $min_order_amount . ' т!';
            $result['message']['error'] = $message_error;
            $result['message']['duration'] = 9000;
            return $result;
        }
    ...
    function (reason, options) { // fail
        ...
        setTimeout(function(){
            window.location='{$url_pay_fail}'
        }, 2000);
	});
JS;
                }else{
                    $result['message']['success'] = 'Ваш заказ успешно отправлен!';
                    $result['js'] = <<<JS
window.location='{$url}'
JS;
                    /**
