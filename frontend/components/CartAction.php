...
JS;
    ...
    public function UpdateSum($items, $new_id = [], $type = 'items', $code_model = false)
    {
        /**
         * @var Items[] $db_items
...
//                ->select(['price', 'id', 'name', 'measure'])
            ...
            foreach ($db_items as $item_id => $item) {
                ...
                $prices_test[] = [
                    'orig_price_by_weight' => $orig_price_by_weight,
                    'full_price_item' => $full_price_item
                ];
...
//            d::ajax($sum_without_discount);
        ...
        $max_price_delivery = (new Delivery())->getCostFreeDelivery(\Yii::$app->session->get('city_select', 1));
        $data['price_delivery_popup'] = 0;

        if ($sum <= \Yii::$app->settings->get('max_price_delivery') && $max_price_delivery > 0) {
            $data['price_delivery_popup'] = number_format($max_price_delivery - $sum, 0, '', ' ') . ' т.';
        }

        if ($max_price_delivery > 0) {
    ...
    public function editBasket()
    {
        ...
        if (!empty($itemsCart)) {
            $items = Items::find()
                ->indexBy('id')
                ->where(['id' => array_keys($itemsCart)])
                ->with(['itemsTypeHandlings.typeHandling', 'itemsTogethers'])
                ->all();

            if (!empty($items)) {
                foreach ($items as $item) {
                    $count = $itemsCart[$item->id];
                    $weight += $item->weight * $count;
                }
            }
        }

        $delivery = (new Delivery())->getDelivery($sum_full, $weight, (int)$city);

        if ($delivery > 0) {
            $result['delivery'] = '<b>' . number_format($delivery, 0, '', ' ') . ' т.</b>';
        }
        elseif ($delivery == 0) {
            $result['delivery'] = '<i class="free">Бесплатная</i>';
        }
        else {
            $result['delivery'] = 'Только самовывоз';
        }

        $sum_full += ($result['delivery'] ? $delivery : 0);
...
//        d::ajax($get);
        ...
        if (($code = \Yii::$app->request->get('code'))
            ...
            $promo_codes_first_order_app = PromoCode::find()
                ->where(['type' => 'first_order_app'])
                ->indexBy('code')
                ->asArray()->all();

            /*
             * Если в GET есть code, то проверим - является ли он
             * кодом активации для первого заказа из приложения.
             * Нужно чтобы эти промокоды не действовали через браузер.
             * Если введёный промокод является промокодом для приложения,
             * а т.е. промокод для первого заказа из приложения, то вернём ошибку.
             */
            if(
                $code AND $promo_codes_first_order_app
                AND in_array($code, array_keys($promo_codes_first_order_app))
            ){
                return [ 'errors' => 'Промокод не действителен!' ];
            }

            $items = Yii::$app->session->get('items', []);
            $this->enable_discount = false;
            $result = $this->UpdateSum($items, [], 'items', $code_model);

            if($result === false){
                return [
                    'errors' => 'Минимальная сумма заказа должна быть: ' . $code_model->min_amount . '!'
                ];
            }

            $sum_full = (int)$result['sum_int'];
            $full_sum_without_discount    = (int)$result['sum_without_discount'];
            $full_sum_with_discount       = (int)$result['sum_with_discount'];
            //d::ajax($full_sum_without_discount);

//            $discount_price = $code_model->discount($sum_full);
            $discount_price = $code_model->discount($full_sum_without_discount);
            //d::ajax($discount_price);
