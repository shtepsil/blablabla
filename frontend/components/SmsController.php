...
class SmsController extends Controller
{
    ...
    const SMSC_LOGIN = 'kingfisherkz';
	//const SMSC_LOGIN = 'login';
	const SMSC_PASSWORD = 'Kingfisher2020';
	const SMSC_POST = 0;
	const SMSC_HTTPS = 0;
	const SMSC_CHARSET = 'utf-8';
	const SMSC_DEBUG = 0;
	const SMTP_FROM = 'api@smsc.kz';
	...
	public static function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = "Kingfisher", $query = "", $files = array())
	{ 
	...
	public function send_sms_mail($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = "Kingfisher")
	{
	...
	public function get_sms_cost($phones, $message, $translit = 0, $format = 0, $sender = "Kingfisher", $query = "")
	{
		static $formats = array(1 => "flash=1", "push=1", "hlr=1", "bin=1", "bin=2", "ping=1", "mms=1", "mail=1", "call=1", "viber=1", "soc=1");

		$m = SmsController::_smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).
						($sender === false ? "" : "&sender=".urlencode($sender)).
	...
	{
		$m = SmsController::_smsc_send_cmd("status", "phone=".urlencode($phone)."&id=".urlencode($id)."&all=".(int)$all);
	...
	{
		$url = $_url = (self::SMSC_HTTPS ? "https" : "http")."://smsc.kz/sys/$cmd.php?login=".urlencode(self::SMSC_LOGIN)."&psw=".urlencode(self::SMSC_PASSWORD)."&fmt=1&charset=".self::SMSC_CHARSET."&".$arg;
