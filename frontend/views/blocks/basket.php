...
if ($context->cart_sets) {
    $q = new ActiveQuery(Sets::className());
    $q->indexBy('id')
        ->andWhere(['id' => array_keys($context->cart_sets)]);
    $sets = $q->all();
};
$sum = $sum_normal = 0;
$content_items = '';
...
<div class="topCart" id="cartWindow">
    ...
    <span class="cart_count_int"><?= $context->cart_count ?></span>
    <div class="popup" id="modalCartWindow">
        <div class="topTitle">
            <div class="line cart_count_string"><?= Yii::t('main', 'count_items', ['n' => $context->cart_count]) ?> на сумму <b class="cart_sum_string"><?= number_format($sum, 0, '', ' ') ?> т.</b></div>
        </div>
        ...
        <div class="bottomTitle">
            <div class="result">
                ...
                <div class="price cart_sum_string"><?= number_format($sum + $discount_price, 0, '', ' ') ?> т.</div>
            </div>
            <div class="result <?= ($discount_price) ? '' : 'hidden' ?>">
                <div class="ttRR">Скидка</div>
                <div class="price cart_small_discount"><?= number_format($discount_price, 0, '', ' ') ?> т.</div>
            </div>
            <div class="result">
                <div class="ttRR">Итого</div>
                <div class="price cart_small_full"><?= number_format($sum_full, 0, '', ' ') ?> т.</div>
            </div>
            <div class="result">
