...
<div id="map"></div>
<?
$coords = explode(',', $context->settings->get('map_coordinates', '54.40787954088725, 86.30976352119039'));
if (isset($coords[0]) && isset($coords[1])) {
    $one = Json::encode(trim($coords[0]), JSON_NUMERIC_CHECK);
    $two = Json::encode(trim($coords[1]), JSON_NUMERIC_CHECK);
    $text_map = Json::htmlEncode($context->settings->get('map_text', 'Тут живу я, Сергей Бражников :)'));
...
//															type: 'yandex#satellite'
        ...
        var placemark1 = new ymaps.Placemark([{$one}, {$two}], {
            ...
            balloonContentLayout: ymaps.templateLayoutFactory.createClass( content, {})
        });
