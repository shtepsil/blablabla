...
class IcmlHelper
{
    ...
    public function __construct($shop, $file = 'retailcrm.xml')
    {
        $path = pathinfo(__FILE__);
        $this->saveDir = str_replace('common/components/retailcrm', 'retailcrm/data/upload', $path['dirname']);
        $this->shop = $shop;
        $this->file = $this->saveDir.'/'.$file;
        $this->tmpFile = sprintf('%s.tmp', $this->saveDir.'/'.$file);
    }
    ...
    public function getSyncDate() 
    {
        ...
        if (file_exists($this->file)) {
            ...
            $syncDate = date('d.m.Y H:i:s', strtotime($attributes->date));
        }
    ...
    private function addOffers($offers)
    {
        ...
        foreach ($offers as $key => $offer) {
            ...
            if (array_key_exists('unit', $offer) && !empty($offer['unit'])) {
                $unit = $e->addChild('unit');
                $unit->addAttribute('code', $offer['unit']['code']);
                $unit->addAttribute('name', htmlspecialchars($offer['unit']['name']));
                $unit->addAttribute('sym', htmlspecialchars($offer['unit']['sym']));
            }
