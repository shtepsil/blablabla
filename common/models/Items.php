...
//use shadow\behaviors\SluggableBehavior;
 ...
 * @property integer $wholesale_price2
...
class Items extends \shadow\SActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['body', 'feature', 'storage', 'delivery'], 'string'],
            ...
            [['name', 'article', 'video', 'tags'], 'string', 'max' => 255],
            [['tags'], 'string', 'max' => 80],
            [['body_small'], 'string', 'max' => 500],
            [['typeHandlings', 'categories', 'seo'], 'safe'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'wholesale_price2' => 'Оптовая цена 2',
            'count' => 'Количество',
            'isVisible' => 'Видимость',
    ...
    {
        return $this->hasOne(Seo::class, ['owner_id' => 'id'])->where(['type' => 'item']);
    }
    public function setSeo($updateSeo)
    {
        if(!$updateSeo){
            $updateSeo=[];
        }
        $event_after = $this->isNewRecord ? $this::EVENT_AFTER_INSERT : $this::EVENT_AFTER_UPDATE;
        $this->on($event_after, function ($event) use ($updateSeo) {
            Yii::trace('start saveSeo');
            $seo = $this->seo;
            if (!$seo) {
                $seo = new Seo();
            }
            ...
            $seo->type        = 'item';
            $seo->title       = ArrayHelper::getValue($updateSeo, 'title');
            $seo->description = ArrayHelper::getValue($updateSeo, 'description');
            $seo->keywords    = ArrayHelper::getValue($updateSeo, 'keywords');
            $seo->save(false);
        });
    }
    /**
    ...
    public function behaviors()
    {
        $result = [
            ...
            [
                'class' => \shadow\behaviors\MySluggableBehavior::className(),
                'attribute' => 'name',
                'slugAttribute' => 'slug',
            ],
        ];
        ...
        if(!in_array(Yii::$app->user->getIdentity()->role,Yii::$app->params['edit_fields'])){
            $params_edit_field = [
                'field_options'=>[
                    'options'=>[
                        /*
                         * div[class=form-group], тут указываются его атрибуты.
                         * Его родитель: div[panel-body]
                         *
                         */
...
JS
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        //$this->updateAttributes(['slug']);
        if (!array_key_exists('updated_at_from_one_c', $changedAttributes)) {
...
//            }
    ...
    public function AllAssociated()
    {
        ...
        $items = ItemAssociated::find()->where(['or', ['item_id_main' => $this->id]])->all();
...
//			'slug' => $this->slug
    ...
    public function origPriceByWeight($count)
    {
        $this->setPriceWholesale();
    ...
    {
        /*
         * Если текущий пользователь оптовик,
         * то устанавливаем цену соответствующего оптовика.
         */
        $this->setPriceWholesale();
        // Применяем персональную скидку для оптовика, если такая есть.
        $this->personalDiscountWholesale();
        if ($this->discount) {
            $result_price = round((int)$this->price * (100 - (int)$this->discount) / 100);
    ...
    public $weight_personal_discount = NULL;
    public $count_personal_discount = NULL;
    public function sum_price($count = 1, $type = 'main', $price = 0, $weight = 0)
    {
        $this->count_personal_discount = $count;
//        d::fajax('-) ' .  $this->price . ' - ' . $count . ' - elseif (-');
        if ($this->measure_price != $this->measure) {
            if (!($weight && $this->measure_price == 0)) {
                ...
                $count = $weight;
//                d::ftdfa('-) ' . $this->price . ' - ' . $count . ' (-');
            }
        }
        $this->weight_personal_discount = $count;
        if ($type == 'purch' && $this->purch_price) {
            $result = round($this->purch_price * $count);
        } elseif ($price) {
//            d::fpri('sum_price->elseif');
//            d::fajax('sum_price->elseif');

//            d::fpri($this->price);
//            d::pri(round($price * $count));
//            d::ftdfa('-) ' .  $this->price . ' - ' . $count . ' - elseif (-');
//            d::fajax('-) ' .  $this->price . ' - ' . $count . ' - elseif (-');
            $result = floor($price * $count);
        } else {
//            d::fpri('sum_price->else');
//            d::fajax('sum_price->else');

//            d::fajax($this->price);
//            if($this->id == 68) d::ftdfa('-) ' .  $this->price . ' - ' . $count . ' - else (-');
            if($this->weight_personal_discount != 1){
                $result = $this->real_price() * $count;
            }else{
                $result = $this->real_price();
            }
//            d::fpri($this->price);
//            d::fajax($result);
        }
        // Сбросим вес... ))
        $this->weight_personal_discount = 1;
        // Сбросим количество
        $this->count_personal_discount = 1;

        return $result;
    }

    /**
     * Проверка на оптовика с настройкой текущей цены для оптовика.
     * @return bool
     */
    function setPriceWholesale()
    {
        $user_type = 0;
        // Сначала зададим тип пользователя $user_type
        /*
         * Тут проверяется - на обычного пользователя.
         * В таблице SUser поле isWholesale отсутствует.
         * т.е. если текущий пользователь не является пользователем админки.
         */
        if(isset(Yii::$app->user->identity->isWholesale)){
            // Получаем поле isWholesale у авторизованного пользователя
            $user_type = Yii::$app->user->identity->isWholesale;
        }
        if(User::$user_type !== false AND User::$user_type > 0){
            // Получаем настроенный тип пользователя (т.е. заранее задаём его, чтобы тут его получить)
            $user_type = User::$user_type;
        }

        // Далее по типу пользователя возьмём соответствующую цену.

        if($user_type == 2 AND $this->wholesale_price2){
            $this->price = $this->wholesale_price2;
        }
        if($user_type == 1 AND $this->wholesale_price){
    ...
    public $personal_discount_status = false;
    public $personal_discount_item = 0;
    public function personalDiscountWholesale()
    {
        // Получаем пользователя, для которого рассчитывается цена на товар.
        $user = User::getCurrentUser();
        if($user != null AND isset($user->personal_discount) AND $user->personal_discount){
            if(isset($user->personal_discount[$this->id])){
                // Если в $discount есть знак %
                if(preg_match('/%$/', $user->personal_discount[$this->id])){
                    /*
                     * Скидку самого товара подменяем скидкой,
                     * которая указана для оптовика
                     */
                    $this->discount = $user->personal_discount[$this->id];
                    $this->personal_discount_item = $user->personal_discount[$this->id];
                }else{
                    /*
                     * Если в строке скидки нет знака %,
                     * то нужно от цены минусовать СУММУ скидки, а не процент от цены.
                     */

                    /*
                     * Добавил сравнение цены со старой ценой,
                     * не пойму, почему после вызова $item->real_price из MicroData
                     * переменная $this->personal_discount_status сохраняет свой статус,
                     * а цена $this->price не сохраняется..
                     */

                    if(!$this->personal_discount_status OR $this->old_price == $this->price){

                        $this->discount = 0;
                        if($user->personal_discount[$this->id] != ''){
                            $this->old_price = $this->price;
//                            d::fajax($this->count_personal_discount);
                            if($this->weight_personal_discount == NULL){
                                $this->weight_personal_discount = 1;
                            }
                            if($this->count_personal_discount == NULL){
                                $this->count_personal_discount = 1;
                            }

                            // От оптовой цены МИНУС число скидки
                            $this->price -= $user->personal_discount[$this->id];
                        }
                        $this->personal_discount_item = $user->personal_discount[$this->id];
                        // Персональная скидка уже применена
                        $this->personal_discount_status = true;
                    }
                }
            }
        }
    ...
    public function check_wholesale_old()
    {
