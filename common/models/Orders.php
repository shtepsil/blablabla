...
use common\validators\OrdersUserExist;
use Yii;
...
class Orders extends \shadow\SActiveRecord
{
    ...
    {
        return [
            [['user_id', 'full_price', '!full_purch_price', '!bonus_use', 'bonus_add', '!status', '!manager_id', '!driver_id', '!collector_id', 'price_delivery', 'isEntity', 'pickpoint_id'], 'integer'],
            [['user_name', 'user_phone', 'user_address', 'date_delivery', 'time_delivery', 'payment', 'pickpoint_id'], 'required'],
            [['user_comments'], 'string'],
            [['user_name', 'user_phone', 'user_mail', 'time_delivery', 'code', 'coordinates_json_yandex', 'claim_id', 'hand_link'], 'string', 'max' => 255],
            [['user_address'], 'string', 'max' => 500],
            [['payment', 'change_status'], 'string', 'max' => 50],
            [['date_delivery', 'admin_comments'], 'safe'],
            [['city_id', 'isWholesale', 'isPhoneOrder', 'delivery_method', 'version_edit', 'isDeadline', 'isApp'], 'integer'],
            [['!full_price'], 'safe'],
            ['bonus_use', 'default', 'value' => 0],
            [['discount'], 'string', 'max' => 255, 'on' => ['admin']],
            [['enable_bonus'], 'integer', 'on' => ['admin']],
            [['manager_id', 'driver_id', 'collector_id', 'id_1c', 'enable_bonus', 'bonus_use'], 'safe', 'on' => 'admin'],
            [['driver_id', 'id_1c'], 'safe', 'on' => 'collector'],
            [['enable_bonus'], 'integer', 'on' => ['manager']],
            [['enable_bonus'], 'safe', 'on' => ['manager']],
            [['user_name', 'user_phone', 'user_address', 'time_delivery'], 'filter', 'filter' => 'trim', 'skipOnArray' => true],
            ['user_phone', OrdersUserExist::className(), 'message' => 'Пользователь с таким номером телефона найден'],
            ['user_mail', OrdersUserExist::className(), 'message' => 'Пользователь с таким email найден'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'pickpoint_id'   => 'Пункт самовывоза',
			'hand_link'   	=> 'Ручная ссылка',
			'delivery_method' => 'Наличие Яндекс Доставки',
			'consignment_note_file' => 'Накладная',
        ];
    }

    public function init()
    {
        $this->data_pay_status = Orders::payStatuses();
    }
    ...
    {
        return $this->hasMany(OrdersItems::className(), ['order_id' => 'id'])->indexBy('item_id');
    }
    /**
     * @return \yii\db\ActiveQuery
     */
    ...
    public function getData_payment()
    {
        return [
            ...
            4 => \Yii::$app->settings->get('payment_type_invoice') ,
        ];
    }
    /**
    ...
    public function behaviors()
    {
        return [
            ...
            'consignment_note' => [
                ...
                'attributes' => [ 'consignment_note_file' ],
            ],
        ];
    }

    /**
     * Добавление в БД пути до файла накладной
     * @param $event
     * @return bool|void
     */
    ...
    /**
     * Удаление из БД пути файла накладной
     * @return bool
     */
    public function afterFileDelete()
    {
...
HTML;
            ...
            case 'pay_status_text':
                ...
                break;
            case 'payment_text':
                $result = (isset($this->getData_payment()[$this->payment]) ? $this->getData_payment()[$this->payment] : '');
                break;
            case 'created_at':
    ...
    public static function searchTimeRow(&$q, $name, $value)
    {
        if ($value !== '') {
            ...
            if (is_numeric($start)) {
                $date = \DateTime::createFromFormat('d/m/Y H:i:s', date('d/m/Y', $start / 1000) . ' 00:00:00', new \DateTimeZone(Yii::$app->timeZone));
                $q->andWhere(['>=', '`orders`.' . $name, $date->getTimestamp()]);
            }
            if (is_numeric($end)) {
                $date = \DateTime::createFromFormat('d/m/Y H:i:s', date('d/m/Y', $end / 1000) . ' 23:59:59', new \DateTimeZone(Yii::$app->timeZone));
                $q->andWhere(['<=', '`orders`.' . $name, $date->getTimestamp()]);
            }
        }
    }
    ...
    public function FormParams()
    {
        $main = [
            ...
            'pickpoint_id'     => [
                'type' => 'dropDownList',
                'data' => ArrayHelper::map(\common\models\Pickpoint::find()->all(), 'id', 'name'),
            ],
        ];
        if ($this->isNewRecord) {
            $main['user_id'] = [
    ...
    public function UpdateOrderItems($items = [], $sets = [], $force = false)
    {
        ...
        if ($items) {
            ...
            foreach ($items as $key => $value) {
                // В этом if обрабатываются товары, которые уже есть в заказе.
                if (isset($old_items[$key])) {
                    ...
                    $full_item_price    = $functions->full_item_price($discount, $target_item, $order_item->count, $order_item->weight);
                    $sum                += $full_item_price;
                    $full_purch_price   += $target_item->sum_price($order_item->count, 'purch', 0, $order_item->weight);
                    $full_bonus_manager += $target_item->full_price_bonus_manager($order_item->count, $order_item->weight, $discount);
//                    $d[] = $order_item->price . ' - if';
                ...
                // Здесь в else, обрабатываются добавленные товары в заказ.
                    $target_item = $db_items[$key];
                    if (isset($value['price'])) {
                        if ($target_item->discount) {
                            $target_item->discount = 0;
                    ...
                    $insert[]           = [
                        ...
                        'weight'        => $weight,
                        'purch_price'   => $target_item->purch_price,
                        'bonus_manager' => $bonus,
                    ...
                    $full_purch_price   += $target_item->sum_price($value['count'], 'purch', 0, $weight);
                    if (isset($value['type_handling'])) {
                        foreach ($value['type_handling'] as $type_handling) {
                            if (!isset($old_type_handling[$type_handling])) {
...
//        if(count($d)) d::ajax($d);
//        d::ajax($insert);
//        d::ajax('common/models/Orders->UpdateOrderItems');

        /**
         * @var $order_set  OrdersSets
         * @var $old_sets   OrdersSets[]
         * @var $target_set Sets
         */
        $insert_sets = [];
        $old_sets    = $this->getOrdersSets()->indexBy('set_id')->all();
        if ($sets) {
            $db_sets = Sets::find()->indexBy('id')->where(['id' => array_keys($sets)])->all();
            foreach ($sets as $key => $value) {
                if (isset($old_sets[$key])) {
                    $order_set          = $old_sets[$key];
                    $order_set->count   = $value['count'];
                    $sum                += round($order_set->count * $order_set->price);
                    $full_bonus_manager += round($order_set->count * $order_set->bonus_manager);
                    $full_purch_price   += round($order_set->count * $order_set->purch_price);
                    $order_set->save(false);
                    unset($old_sets[$key]);
                } else {
                    ...
                    $insert_sets[]      = [
                        ...
                        'count'         => $value['count'],
                        'price'         => $target_set->real_price(),
                        'purch_price'   => $target_set->real_purch_price(),
                        'bonus_manager' => $bonus
                    ];
                    $full_purch_price   += round($value['count'] * $target_set->real_purch_price());
                    $sum                += round($value['count'] * $target_set->real_price());
                    $full_bonus_manager += round($value['count'] * $bonus);
                }
            }
        }
        $this->full_purch_price = $full_purch_price;
        $this->full_price       = floor($sum);
    ...
    public $data_status = [
        ...
        2  => 'Заказ на подтверждение клиентом',
    ...
    public function statusToKey($status = null)
    {
        $data = [
            0  => 'new',
            1  => 'shaping',
            ...
            4  => 'delivery',
            5  => 'success',
            6  => 'rollback',//TODO не создавал класс
            7  => 'partial_return',//TODO не создавал класс
            8  => 'fail',
            9  => 'not_responding',
            10 => 'recovery',
			11 => 'double'
...
JS;
    ...
    public function beforeSave($insert)
    {
        if (!$this->isNewRecord && $this->getAttribute('bonus_use') != $this->getOldAttribute('bonus_use')) {
            ...
            Yii::$app->db->createCommand()->insert('s_log_action', [
    ...
    public $send_message_status = false;
    /**
    ...
    public function commitBonus()
    {
        if ($this->user_id && $this->enable_bonus) {
            ...
            User::updateAllCounters(['order_sum' => ($this->full_price - $this->discount($this->full_price))], ['id' => $this->user_id]);
        }
        if ($this->manager_id) {
    ...
    public function convert_all_to_model($old_order_items, $db_items)
    {
        if (isset($old_order_items[0])) {
            $old_order_items = ArrayHelper::map($old_order_items,
                function ($el) {
                    return $el->item_id;
                },
                function ($el) {
    ...
    {
        return (($this->full_price + (integer)$this->price_delivery) - $this->discount($this->full_price)) - $this->bonus_use;
    }
    ...
    public function sendAddPay($sum)
    {
        $request = [
            'Amount'              => $sum,
            'Currency'            => 'KZT',
            'Description'         => 'Оплата заказ на сайте kingfisher.kz',
            'InvoiceId'           => $this->id,
        ...
        if ($this->user_mail) {
            $request['Email']     = trim($this->user_mail);
    ...
    public function successPay()
    {
        ...
        $payments = OrdersPay::find()->where(['order_id' => $this->id, 'status' => [0, 1]])->orderBy(['status' => SORT_DESC])->all();
            ...
            if (!$sum && $payment->status == 0) {
                ...
                if ($payment->status == 0) {
                    ...
                    if ($sum >= floatval($payment->amount)) {
                        ...
                        OrdersPay::updateAll(['status' => 1, 'real_amount' => $payment->amount], ['id' => $payment->id]);
                    }else if($sum < floatval($payment->amount)){
                        Orders::sendSuccessPay($payment->id, $sum);
                        OrdersPay::updateAll(['status' => 1, 'real_amount' => $sum], ['id' => $payment->id]);
                        $sum = 0;
                        continue;
                    } else {
                        Orders::sendSuccessPay($payment->id, floatval($payment->amount) - $sum);
                        OrdersPay::updateAll(['status' => 1, 'real_amount' => floatval($payment->amount) - $sum], ['id' => $payment->id]);
                    }
                }
     ...
     */
    public static function checkPayStatus($order)
    {
        $request  = [
            'InvoiceId' => $order->id,
        ];
        $result   = false;
        $response = Orders::sendRequest('/payments/find', $request);
        if (isset($response['Success']) && $response['Success']) {
            $TransactionId = $response['Model']['TransactionId'];
            $Amount        = $response['Model']['Amount'];
            if (!OrdersPay::find()->where(['id' => $TransactionId])->exists()) {
                \Yii::$app->db->createCommand()->insert(OrdersPay::tableName(), [
                    'id'       => $TransactionId,
                    'order_id' => $order->id,
                    'amount'   => $Amount,
                    'status'   => ($order->status == 5 ? 1 : 0)
                ])->execute();

            }
            if ($order->pay_status == 'wait_surcharge') {
                $order->pay_status = 'success_surcharge';
    ...
    public static function sendRequest($endpoint, array $params = [])
    {
        ...
        curl_setopt($curl, CURLOPT_USERPWD, sprintf(
            '%s:%s',
            Yii::$app->params['cloudpayments']['public_id'],
            Yii::$app->params['cloudpayments']['api_key']
        ));
    ...
    {
        if (!$this->_all_items) {
            $items = OrdersItems::find()->with('item')->where(['order_id' => $this->id])->all();
            foreach ($items as $key => &$value) {
                $value->populateRelation('item', $this->convert_to_model($value, $value->item));
            }
            $this->_all_items = array_merge($items, $this->ordersSets);
        }
        return $this->_all_items;
    }
	
	public function getCountOrdersThisMounth($user_id) {
		$last = time() - (30 * 24 * 60 * 60);
		$count_orders = Orders::find()
		->andWhere(['user_id' => $user_id])
		//->andWhere(['pay_status' => 'success'])
		->andWhere(['status' => 5])
		->andWhere(['>', 'created_at', $last])->count();
		return $count_orders;
	}
	
	public function getSucessOrdersUsers($user_id) {

		$count_orders = Orders::find()
		->andWhere(['user_id' => $user_id])
		->andWhere(['status' => 5])->count();
		//->andWhere(['pay_status' => 'success'])->count();
