...
<? if (!count($requisites)): ?>
    ...
    <center>
        <h3>Внимание</h3>
        <font color="red">
            Текущий пользователь, сделавший заказ, не имеет реквизитов.
        </font>
    </center>
    ...
    <div class="main">
        <table width="100%" style="font-family: Arial;">
            ...
            <tr>
                ...
                <td style="width: 32%; text-align: center; padding: 30px 0;display:none;"><img src="<!--Лого url-->"
                                                                                               style="width: 70%;"></td>
            </tr>
        ...
        <table width="100%" border="2" style="border-collapse: collapse; width: 100%; font-family: Arial;">
            ...
            <tr style="">
                <td valign="top" style="font-weight:bold;">
                    <div style="padding-left: 10px;">Бенефициар:</div>
                </td>
        ...
        <table border="2" width="100%" cellpadding="2" cellspacing="2"
            ...
            <? if (count($order_items)):
                ...
                <tr>
                    ...
                    <td style="width:17mm;text-align:center;"><?=($order_item['item']->measure_price == 0) ? 'кг' : 'шт' ?>
                    </td>
                    <td style="width:27mm; text-align: center; ">
                        <?= number_format($order_item['item']->price, 2, ',', ' ') ?></td>
                    <td style="width:27mm; text-align: center; ">
                        <?= number_format($order_item['item_full_price'], 2, ',', ' ') ?>
                    </td>
                </tr>
            <? endforeach ?>
            <? endif ?>
        ...
        <table style="font-family: Arial;" border="0" width="100%" cellpadding="1" cellspacing="1">
            <tbody>
            <tr>
                <td colspan="3" style="width:27mm; font-weight:bold;text-align: right;">Итого:
                    <?= number_format($order_full_sum, 2, ',', ' ') ?>
                </td>
            </tr>
            <tr>
                <td style="width:27mm; font-weight:bold;text-align: right;">В том числе НДС:
                    <?= number_format( $functoins->nds($order_full_sum),
                        2, ',', ' '
                    ) ?>
                </td>
            </tr>
        ...
        <div style="font-family: Arial;">
            Всего наименований
            <?= $counter ?>, на сумму
            <?= number_format($order_full_sum, 2, ',', ' ') ?>     <?= Yii::$app->params['currency'] ?>.
        </div>
        <div style="font-weight:bold;font-family: Arial;margin-bottom:5px;">
            Всего к выплате:
            <?= StringHelper::cucfirst(SNumberHelper::num2str($order_full_sum)) ?>
        </div>
        ...
        <div style="font-family: Arial; font-size: 10pt;display:none;">
            1. Счет действителен в течении 5 (пяти) банковских дней, не считая дня выписки счета. В случае нарушения срока
            оплаты сохранение цены на товар и наличие товара на складе НЕ ГАРАНТИРУЕТСЯ.<br>
            2. Оплата данного счета означает согласие с условиями изложенными в п.1</div>
        <div
                style="background: url('<!--url печати в png сюда-->');  background-repeat: no-repeat; padding: 20px 10px; width: 700px; height: 250px;">
