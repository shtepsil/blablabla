...
class MessageSend extends Model
{
    ...
    public function send()
    {
        ...
        $send_mails = explode(',', \Yii::$app->settings->get('admin_email', 'viktor@instinct.kz'));
        ...
        if ($mailer->send()) {
            if(\Yii::$app->params['RetailCRM']['enable'] === true){
                ...
                $crmOrder->createdAt = date('Y-m-d H:i:s');
                $crmOrder->firstName = $attrs['name'];
                $crmOrder->phone = $attrs['phone'];
                $crmOrder->email = $attrs['email'];
                $crmOrder->customerComment = $attrs['message'];
                switch ($attrs['form']) {
                    case 'opt':
                        $crmOrder->orderMethod = 'message-opt';
                        break;
                    case 'contact':
                        $crmOrder->orderMethod = 'message-contacts';
                }
                $crmApiHelper = new ApiHelper();
                $crmApiHelper->createOrder($crmOrder->attributes);
            }
            $result['message']['success'] = 'Ваше сообщение успешно отправлено!';
            $result['js'] = "$('.formContacts').resetForm()";
        } else {
