...
class SUrlRule extends BaseObject implements UrlRuleInterface
{
    /**
    ...
    public function parseRequest($manager, $request)
    {
        ...
        // ==========================================
        // Костыль, для редиректов со старых ссылок на новые ЧПУ

        // Получим GET параметры
        $get = $request->getQueryParams();
        // Если в GET есть параметр id
        if(array_key_exists('id',$get)) {
            // Составим строку для запроса WHERE
            $pathInfo = $pathInfo.'?id='.$get['id'];
        }else{
            /*
             * .htaccess в конец всех ссылок добавляет слэш
             * А так как в таблице s_seo_redirects старые ссылки
             * хранятся без слэша на конце, то из $pathInfo
             * нужно удалить все слэши сначала и конца строки
             */
...
//            }
            ...
            /*
             * Если в таблице редиректов ничего не найдено,
             * то обновим переменную $pathInfo,
             * чтобы дальше ничего не сломалось из-за добавленного костыля
             */
            $pathInfo = $request->getPathInfo();
...
//        d::pri($return);
     ...
     * @inheritdoc
     */
    public function createUrl($manager, $route, $params,$test=false)
    {
        $route = trim($route, '/');
        $url = false;
...
//            if(isset($this->_createUrlCache[$key_cache])){
                ...
                /** @var SSeoUrls $item */
//                d::tdfa($q->createCommand()->sql);
//                d::tdfa($resource_id.' - '.$controller.' - '.$action);
//                d::tdfa('---------');
                $item = $q->one();
                if ($item) {
                    $url = $item->path;
