...
class SSeoBehavior extends Behavior
{
    ...
    public static function enableSeoEdit()
    {
	   ...
	   return
		(Yii::$app->id == 'app-backend' && in_array($arr, ['control', 'save', 'deleted']))
		&&
		(Yii::$app->seo->enable)
		;
    }
...
JS
    ...
    public function saveSeo($seo = false)
    {
        ...
        /*
         * При создании нового элемента, который имеет поле Seo Url,
         * добавляем редирект со старой ссылки на ЧПУ
         * Пример:
         * Старая ссылка: page.html/?id=7
         * Новая ссылка: novaya-stranica
         */
        $redirect = new SSeoRedirects();
        /*
         * Пример:
         * $old_path = $this->action.'.html/?id='.$owner->getPrimaryKey()
         * $new_path = $this->getSeoAttribute('seo_path')
         */
        $old_path = $this->action.'.html/?id='.$owner->getPrimaryKey();
        // Редирект на ЧПУ ссылку
        $new_path = $this->getSeoAttribute('seo_path');
        $redirect->old_url = $old_path;
        $redirect->new_url = $new_path;
        $redirect->type = '301';
