...
class SaveRelationBehavior extends SBehavior
{
    ...
    public $settings = [
        'img' => [
            'filePath' => '@web_frontend/uploads/[[model]]/[[filename]].[[extension]]',
            'fileUrl' => '/uploads/[[model]]/[[filename]].[[extension]]',
        ],
        'relation' => [],
        'MANY_MANY' => [],
    ];
    ...
    public function saveImg($relation, $items, $column, $extra_attributes = [])
    {
        ...
        $save_path = Yii::getAlias('@web_frontend/uploads/' . $relation);
        ...
        if ($data) {
            ...
            foreach ($data as $key => $img) {
                if (!isset($items[$key]) && file_exists($tmp_path . $img['url'])) {
